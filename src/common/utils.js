...
const prettifyCurrency = (
  ...
  return cryptocurrency
    ? `${symbolPrepended ? symbol + '\u00A0' : ''}${currencyValue.format(
        amount,
      )}${symbolPrepended ? '' : '\u00A0' + symbol}`
...
const searchFor = (
  ...
  const object = array.reduce((acc, item, index) => {
    if (Object.keys(item).find(key => String(item[key]).match(regEx)))
      acc[index] = item
    return acc
  }, {})
  if (callBack) callBack(Object.values(object))
...
const addUnknownTokenToList = (tokenList, newToken) => {
  if (!newToken) return tokenList

  const newList = tokenList
  const index = newList.findIndex(
    token =>
      token.chainId == newToken.chainId &&
...
const getCombinedTokenListFromSources = sources => {
  return Promise.all(
    sources
      ...
      .map(source => {
        const p = getTokenList(source.url).then(d =>
          d.tokens.filter(token => token.chainId == defaults.network.chainId),
        )
        return p
      }),
  ).then(data => {
...
const calculateDifference = (value1, value2) =>
  value1 > value2 ? (value1 - value2) / value2 : (value1 - value2) / value1
