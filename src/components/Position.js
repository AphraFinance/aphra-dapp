...
export const Position = props => {
  ...
  useEffect(() => {
    getTokenByAddress(props.foreignTokenAddress).then(t => {
      setToken(t)
    })
    return () => setToken({})
  ...
  return (
    <Link
      to={`/pool/position/${props.position.foreignAsset.address}/${props.position.id}`}
    >
      <Flex
        width="100%"
        alignItems="center"
          ...
          {props.position.originalNative && (
            <Tag colorScheme="blue">
              {prettifyCurrency(
                ethers.utils.formatUnits(
                  props.position.originalNative,
                  defaults.nativeAsset.decimals,
                ),
                0,
                4,
                defaults.nativeAsset.symbol,
              )}
            </Tag>
          )}
        </Flex>
