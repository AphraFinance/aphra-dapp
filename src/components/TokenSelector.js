    ...
    >
      {data.tokenList && (
        <>
          ...
          <Box paddingLeft="34px" fontSize="1rem" fontWeight="100" color="#666">
  ...
  return (
    <>
      ...
      <ModalBody display="flex" flexDir="column">
        <Box p="0 1.5rem 1.5rem" borderBottom="1px solid #00000017">
          <Input
            size="lg"
...
const TokenListSelectDialog = props => {
  ...
  return (
    <>
      <ModalHeader textAlign="center" borderBottom="1px solid #00000017">
        <Button
          variant="ghostDark"
          minW="auto"
          width="32px"
        ...
        >
          <ArrowBackIcon width="18px" height="18px" />
        </Button>
        Manage
        <ModalCloseButton top="1.29rem" />
      </ModalHeader>
      <ModalBody display="flex" flexDir="column" p="1.8rem 0 0">
...
export const TokenSelector = props => {
  ...
  const [tokenListSources, setTokenListSources] = useLocalStorage(
    'tokenListSources',
  ...
  useEffect(() => {
    if (!Array.isArray(props.tokenList)) {
      getCombinedTokenListFromSources(tokenListSources)
        .then(data => {
