...
const Burn = props => {
      ...
      if (!wallet.account) {
        ...
        if (tokenBalance.gte(value)) {
          ...
          if (
            ...
            setWorking(true)
            const proof = getMerkleProofForAccount(
              wallet.account,
  ...
  useEffect(() => {
    if (
      ...
      getClaimed(leaf, defaults.network.provider).then(r => {
            ...
            if (defaults.redeemables[0].snapshot[wallet.account]) {
              if (!vethAllowLess) {
                if (
                  ...
                  setValue(data)
                  setInputAmount(
                    ethers.utils.formatUnits(data, tokenSelect.decimals),
                  )
                }
              }
            }
                  ...
                  <>
                    {!vethAccountLeafClaimed &&
                      ...
                      Number(defaults.redeemables[0].snapshot[wallet.account]) >
                        0 && (
                        <>
                          <Alert m="1rem 0 1rem" status="warning">
                            <AlertIcon />
                            <Box flex="1">
                              <AlertTitle mr={2}>
                                No way to burn multiple times
                              </AlertTitle>
                              <AlertDescription>
                                This token can be burned by eligible account
                                only once. If you decide to burn less than
                                you&apos;re entitled to, you&apos;ll be not able
                                to burn more afterwards and so claim the maximal
                                amount.
                              </AlertDescription>
                            </Box>
                          </Alert>
                    ...
                    {defaults.redeemables[0].snapshot[wallet.account] &&
                      Number(defaults.redeemables[0].snapshot[wallet.account]) >
                        0 && (
                        <Alert m="0 0 1rem" status="info">
                          <AlertIcon />
                          <Box flex="1">
                            <AlertDescription>
                              Vested portion&apos;s linearly released for
                              1&nbsp;year. Claiming has no limits. Can be done
                              regularly at&nbsp;any time.
                            </AlertDescription>
                          </Box>
                        </Alert>
                      )}
                    ...
                    {(!defaults.redeemables[0].snapshot[wallet.account] ||
                      ...
                      <Alert m="0 0 1rem" status="error">
                        <AlertIcon />
                        <Box flex="1">
                          <AlertTitle mr={2}>Not eligible account</AlertTitle>
                          <AlertDescription>
                            Sorry, your account can not burn this token.
                          </AlertDescription>
                        </Box>
                      </Alert>
                    )}
                    ...
                    {wallet.account &&
                      ...
                      Number(defaults.redeemables[0].snapshot[wallet.account]) >
                        0 && (
                        <VethBreakdown
                          claimable={claimableVeth}
                          vethAccountLeafClaimed={vethAccountLeafClaimed}
                        />
                      )}
                  </>
                )}
                            ...
                            if (Number(e.target.value) > 0) {
                              try {
                                if (
                                  tokenSelect.symbol === 'VETH' &&
                                  ethers.utils
                                    .parseUnits(String(e.target.value), 18)
                                    .gt(
                                      defaults.redeemables?.[0].snapshot?.[
                                        wallet.account
                                      ],
              ...
              {wallet.account && (
                <>
                  {!working && tokenSelect && !tokenSelect.symbol === 'VETH' && (
                    <>
                      {!tokenApproved && <>Approve {tokenSelect.symbol}</>}
                      {tokenApproved && <>Claim</>}
                    </>
                  )}
                  {!working && tokenSelect && tokenSelect.symbol === 'VETH' && (
